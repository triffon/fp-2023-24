...
; Нека видим как бихме имплементирали функцията take
; за потоци.
...
; Генерира безкраен поток от елементите на lsts
...
; със stream-map*
(define nats* (cons-stream 0 (stream-map* 1+ nats*)))

; с iterate
(define nats** (iterate 1+ 0))
...
; Връща първото дете на v в g,
; което е вярно за предиката p.
...
; Инвертира графа g. Тоест за всяко ребро (u,v) в g
; новият граф ще има реброто (v,u).
(define (invert g)
